...
- **技能标准化**：15 个技能采用 Agent Skills 开放格式，四端通用。
...
```
① 想法（用户一句话）
   → ② 项目设定（studio + bootstrap-project 技能）
       聊需求 → PROJECT.md → 技术选型 → 派发 backend-dev 脚手架
   → ③ 功能规划（spec-driven propose，每功能）
       需求(proposal) → 设计(UI稿) → 架构(API契约) → 任务拆分(tasks)
   → ④ 实现+测试（dispatch-dev + test-engineer）
       前后端并行 + code-reviewer + 按 spec 持续测试 + UAT
   → ⑤ 部署+发布（devops + test-engineer）
       构建部署 + E2E/负载测试 + 发布决策
   → ⑥ 复盘+Archive（spec-driven archive）
...
- **交互对话由技能在主线程承载。** `studio`、`bootstrap-project`、`spec-driven` 在主线程和用户对话。
...
`bootstrap-project` 技能只做选型，不执行脚手架。脚手架由 tech-lead -> backend-dev 派发。
...
- 用户确认后 → `bootstrap-project` 做技术选型
- 选型确认后 → tech-lead 派发 backend-dev 执行脚手架
- backend-dev 从 `go-ai-backend-template` clone 后端骨架到 `backend/`
...
| Agent | 职责 | 绑定技能 |
|-------|------|---------|
| **project-manager** | 产品需求：聊想法、维护 PROJECT.md、写 proposal | bootstrap-project, spec-driven |
...
| 项目设定 | `bootstrap-project` | 访谈 → PROJECT.md → 定架构 → 写配置 → 派发脚手架 |
...
- **bootstrap-project 只做选型，不操作代码**。脚手架由 tech-lead 派发 backend-dev 执行。
