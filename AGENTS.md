...
1. **想法** — 一句话或一段话描述要做什么。
2. **项目设定** — `studio` 访谈产出 `PROJECT.md`，铺设管道（openspec init + 四端配置），进入功能规划。
...
由 `studio` 访谈产出 `PROJECT.md`，铺设管道（openspec init + 四端配置），然后进入功能规划阶段让 tech-lead 做技术选型。完整规范见 `rules/backend-spec.md` 与 `backend/README.md`（clone `go-ai-backend-template` 后存在）。
...
| 项目设定/架构 | `studio` | 需求访谈→PROJECT.md→管道铺设→引导 spec-driven（tech-lead在③c做选型）|
...
配置见 `.mcp.json` 与 `rules/code-intelligence.md`；由 `studio` 按需写入。
